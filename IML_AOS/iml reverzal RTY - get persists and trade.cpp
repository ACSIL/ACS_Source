...
SCString log_sc_rty(SCStudyInterfaceRef sc)
{
    ...
    rty.entry_price = current_postion.AveragePrice;
    double entry_dt = current_postion.LastFillDateTime;
    DATETIME_TO_YMDHMS(entry_dt, rty.year, rty.month, rty.day, rty.hour, rty.minute, rty.second);
...
{
    std::string file_name{ "RTY trades.txt" };
    std::ofstream out_file{};
    out_file.open(file_name, std::ofstream::app);
    if (out_file.is_open())
    {
        std::string test_logu{ log_sc_rty(sc) }; 
        out_file << test_logu << '\n';
    }
    else
    {
...
SCSFExport scsf_MAIN_RTY_IML_REV_GetPersistsAndTrade(SCStudyInterfaceRef sc)
{
    ...
    if (sc.SetDefaults)
    {
        ...
        sc.GraphRegion = 0;
        sc.GraphName = "main: IML REV RTY - Get persists and trade";
        sc.AutoLoop = 1;
    ...
    rty.range = (daily_high - daily_low) / sc.TickSize;
    if (rty.range < minimal_range.GetInt())
        return;
    ...
    return_val_sum = static_cast<int>(return_val_from_es + return_val_from_nq + return_val_from_ym);
    ...
    if (is_time_to_trade(sc))
    {
        if (bool_ready_to_trade_LONG && sc.BaseDateTimeIn[sc.IndexOfLastVisibleBar] < iml_valid_till_dt && sc.Close[sc.IndexOfLastVisibleBar] <= daily_low_mid)
        {
            ...
            if (entry_check)
            {
                sc.AddMessageToLog(log_sc_rty(sc), 0); // custom foo (internal log)
                log_txt_rty(sc); //  custom function (external log) 
        ...
        else if (bool_ready_to_trade_SHORT && sc.BaseDateTimeIn[sc.IndexOfLastVisibleBar] < iml_valid_till_dt && sc.Close[sc.IndexOfLastVisibleBar] >= daily_high_mid)
        {
            ...
            {
                sc.AddMessageToLog(log_sc_rty(sc), 0); //  custom foo (internal log)
                log_txt_rty(sc); // calling custom function (external log) 
    ...
    if (display_ut.GetYesNo())
    {
        ...
        sum.Region = 0;
        sum.Text.Format("ES: %.0f, NQ: %.0f, YM: %.0f -> SUM: %i | RTY: %.0f  ", return_val_from_es, return_val_from_nq, return_val_from_ym, return_val_sum, return_val_from_rty);
        sum.LineNumber = 1;
        sum.BeginDateTime = 1;
        sum.BeginValue = 93;
        sc.UseTool(sum);
