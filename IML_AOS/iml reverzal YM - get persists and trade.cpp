...
SCString log_sc_ym(SCStudyInterfaceRef sc)
{
    ...
    ym.entry_price = current_postion.AveragePrice;
    double entry_dt = current_postion.LastFillDateTime;
    DATETIME_TO_YMDHMS(entry_dt, ym.year, ym.month, ym.day, ym.hour, ym.minute, ym.second);

    SCString log;
    log.Format("Cena: %f, Date and Time: %i-%i-%i  %0.2i:%0.2i:%0.2i | Daily range (in ticks): %f | ATR: %0.04f",
        ym.entry_price, ym.year, ym.month, ym.day, ym.hour, ym.minute, ym.second, ym.range, ym.atr_val);
    return log;
}
...
{
    std::string file_name{ "YM trades.txt" }; //calling custom function 
    std::ofstream out_file{};
    out_file.open(file_name, std::ofstream::app);
    if (out_file.is_open())
    {
        std::string test_logu{ log_sc_ym(sc) }; //calling custom function 
        out_file << test_logu << '\n';
    }
    else
    {
...
SCSFExport scsf_main_IML_REV_GetPersistsAndTrade(SCStudyInterfaceRef sc)
{
    ...
    if (sc.SetDefaults)
    {
        ...
        sc.GraphRegion = 0;
        sc.GraphName = "main: IML REV YM - Get persists and trade";
        sc.AutoLoop = 1;
    ...
    //geting the values from the context study (break dhl)
    ...
    ym.range = (daily_high - daily_low) / sc.TickSize;
    if (ym.range < minimal_range.GetInt())
        return;
       ...
       if (bool_ready_to_trade_LONG && sc.BaseDateTimeIn[sc.IndexOfLastVisibleBar] < iml_valid_till_dt && sc.Close[sc.IndexOfLastVisibleBar] <= daily_low_mid)
	    {		  		  
    	    ...
    	    t_OrderQuantity32_64 entry_check = sc.BuyEntry(order);
  		    ...
  		    if (entry_check && bool_logged == 0)
		    {
                sc.AddMessageToLog(log_sc_ym(sc), 0); // custom foo (internal log)
                log_txt_ym(sc); //  custom function (external log) 
	    ...
	    else if (bool_ready_to_trade_SHORT && sc.BaseDateTimeIn[sc.IndexOfLastVisibleBar] < iml_valid_till_dt && sc.Close[sc.IndexOfLastVisibleBar] >= daily_high_mid)
	    {
		    ...
		    {
                sc.AddMessageToLog(log_sc_ym(sc), 0); //  custom foo (internal log)
                log_txt_ym(sc); // calling custom function (external log) 
    ...
    if (display_ut.GetYesNo())
	{
		...
		sum.Region = 0;
		sum.Text.Format("ES: %.0f, NQ: %.0f, RTY: %.0f -> SUM: %i | YM: %.0f  ", return_val_from_es, return_val_from_nq, return_val_from_rty, return_val_sum, return_val_from_ym);
		sum.LineNumber = 1;
		sum.BeginDateTime = 1;
		sum.BeginValue = 93;
		sc.UseTool(sum);
