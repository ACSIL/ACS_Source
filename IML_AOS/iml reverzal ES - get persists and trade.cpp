...
SCString log_sc_es(SCStudyInterfaceRef sc)
{
    ...
    es.entry_price = current_postion.AveragePrice;
    double entry_dt = current_postion.LastFillDateTime;
    DATETIME_TO_YMDHMS(entry_dt, es.year, es.month, es.day, es.hour, es.minute, es.second);
...
{
    std::string file_name{ "ES trades.txt" }; //calling custom function 
...
SCSFExport scsf_MAIN_ES_IML_REV_GetPersistsAndTrade(SCStudyInterfaceRef sc)
{
    ...
    if (sc.SetDefaults)
    {
        ...
        sc.GraphRegion = 0;
        sc.GraphName = "main: IML REV ES - Get persists and trade";
        sc.AutoLoop = 1;
    ...
    es.range = (daily_high - daily_low) / sc.TickSize;
    if (es.range < minimal_range.GetInt())
        return;
    ...
    return_val_sum = static_cast<int>(return_val_from_rty + return_val_from_nq + return_val_from_nq);
    ...
    s_SCPositionData current_postion;
    sc.GetTradePosition(current_postion);
    double sl_offset = round(daily_high_mid - daily_low_mid) / sc.TickSize; // TADY JE BUG POCITA TO BLBE SL-
    //set order
    s_SCNewOrder order;
    ...
    if (is_time_to_trade(sc))
    {
        if (bool_ready_to_trade_LONG && sc.BaseDateTimeIn[sc.IndexOfLastVisibleBar] < iml_valid_till_dt && sc.Close[sc.IndexOfLastVisibleBar] <= daily_low_mid)
        {
            ...
            if (entry_check)
            {
                sc.AddMessageToLog(log_sc_es(sc), 0); // custom foo (internal log)
                log_txt_es(sc); //  custom function (external log) 
        ...
        else if (bool_ready_to_trade_SHORT && sc.BaseDateTimeIn[sc.IndexOfLastVisibleBar] < iml_valid_till_dt && sc.Close[sc.IndexOfLastVisibleBar] >= daily_high_mid)
        {
            ...
            {
                sc.AddMessageToLog(log_sc_es(sc), 0); //  custom foo (internal log)
                log_txt_es(sc); // calling custom function (external log) 
    ...
    if (display_ut.GetYesNo())
    {
        ...
        sum.Region = 0;
        sum.Text.Format("RTY: %.0f, NQ: %.0f, YM: %.0f -> SUM: %i | ES: %.0f  ", return_val_from_rty, return_val_from_nq, return_val_from_ym, return_val_sum, return_val_from_es);
        sum.LineNumber = 1;
        sum.BeginDateTime = 1;
        sum.BeginValue = 93;
        sc.UseTool(sum);
