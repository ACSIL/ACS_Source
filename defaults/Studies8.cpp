...
SCSFExport scsf_NarrowRangeBar(SCStudyInterfaceRef sc) {
    ...
    if (sc.SetDefaults) {
        ...
        Input_LookBackType.SetCustomInputStrings("NR n;xBar NR;NR vs ATR");
...
SCSFExport scsf_WideRangeBar(SCStudyInterfaceRef sc) {
    ...
    if (sc.SetDefaults) {
        ...
        Input_LookBackType.Name = "Lookback Type";
        Input_LookBackType.SetCustomInputStrings("WR n;xBar WR;WR vs ATR");
        Input_LookBackType.SetCustomInputIndex(0);
        Input_LookBackType.DisplayOrder = ++InputDisplayOrder;
            ...
            } else  // down column
            {
                ...
                if (PassNumber == 0 || (!UseHighFirst && PassNumber == 1)) {
                    if (sc.FormattedEvaluate(HighInputData[InputBarIndex], sc.BaseGraphValueFormat, GREATER_EQUAL_OPERATOR, Subgraph_Low[OutputIndex] + r_CurrentBoxSize * Input_ReversalSize.GetFloat(), sc.BaseGraphValueFormat)) {
                        ...
                        bool SetOpen = false;
                        if (Input_AllowOneBoxReversals.GetYesNo() || sc.FormattedEvaluate((Subgraph_High[OutputIndex] - Subgraph_Low[OutputIndex]), sc.BaseGraphValueFormat, GREATER_EQUAL_OPERATOR, r_CurrentBoxSize, sc.BaseGraphValueFormat)) {
                            // Calculate the number of boxes for the column
                            Subgraph_NumberOfBoxes[OutputIndex] = (float)sc.Round((Subgraph_High[OutputIndex] - Subgraph_Low[OutputIndex]) / r_CurrentBoxSize + 1);
...
SCSFExport scsf_StudyOverlayOHLC(SCStudyInterfaceRef sc) {
    ...
    if (Input_UseStandardOHLCSubgraphsFromSourceStudy.GetYesNo()) {
        if (ReferenceArrays[SC_OPEN].GetArraySize() == 0 || ReferenceArrays[SC_HIGH].GetArraySize() == 0 || ReferenceArrays[SC_LOW].GetArraySize() == 0 || ReferenceArrays[SC_LAST].GetArraySize() == 0) {
    ...
    for (int OutputArrayIndex = StudyStartIndex; OutputArrayIndex < ArraySize; OutputArrayIndex++) {
        ...
        while (NextIndexDateTime > ReferenceChartDateTimeArray[RefIndex]) {
            if ((Input_UseZeroValuesFromSource.GetYesNo() || (*p_ReferenceHigh)[RefIndex] != 0.0f) && HighVal < (*p_ReferenceHigh)[RefIndex]) {
                HighVal = (*p_ReferenceHigh)[RefIndex];
            } else if ((Input_UseZeroValuesFromSource.GetYesNo() || (*p_ReferenceLow)[RefIndex] != 0.0f) && LowVal > (*p_ReferenceLow)[RefIndex]) {
...
SCSFExport scsf_DailyOHLC(SCStudyInterfaceRef sc) {
    ...
    if (Input_DisplayTotalDailyVolumeOnly.GetYesNo() && sc.Subgraph[SC_OPEN].DrawStyle != DRAWSTYLE_IGNORE) {
    ...
    } else if (!Input_DisplayTotalDailyVolumeOnly.GetYesNo() && sc.Subgraph[SC_OPEN].DrawStyle == DRAWSTYLE_IGNORE && sc.Subgraph[SC_VOLUME].DrawStyle != DRAWSTYLE_IGNORE) {
        sc.Subgraph[SC_OPEN].DrawStyle = DRAWSTYLE_DASH;
        sc.Subgraph[SC_HIGH].DrawStyle = DRAWSTYLE_DASH;
        sc.Subgraph[SC_LOW].DrawStyle = DRAWSTYLE_DASH;
        sc.Subgraph[SC_LAST].DrawStyle = DRAWSTYLE_DASH;
    ...
    if (StartIndex != 0) {
        ...
        if (InIncludeFridayEveningSessionWithSundayEveningSession && TradingDayStartDateTime.IsSunday()) {
    ...
    for (int Index = StartIndex; Index < sc.ArraySize; Index++) {
        ...
        if (InIncludeFridayEveningSessionWithSundayEveningSession && CurrentBarTradingDayDate.IsSaturday()) {
        ...
        if (InGraphHighLowHistorically) {
            if (!Input_UseDaySessionOnly.GetYesNo() || (Input_UseDaySessionOnly.GetYesNo() && sc.IsDateTimeInDaySession(BarDateTime))) {
        ...
        if (Input_UseDaySessionOnly.GetYesNo() && Input_UsePreviousValuesUntilNewDaySession.GetYesNo() && !sc.IsDateTimeInDaySession(BarDateTime) && Index > 0) {
    ...
    for (int Index = StartIndex; Index < sc.ArraySize; Index++) {
        ...
        if (sc.FormattedEvaluate(High, sc.BaseGraphValueFormat, EQUAL_OPERATOR, sc.High[Index], sc.BaseGraphValueFormat)) {
            ...
            if (InDrawExtensionLines) {
                ...
                sc.AddLineUntilFutureIntersection(Index, 1, High, Subgraph_ExtensionLinePropertiesSubgraph.PrimaryColor, Subgraph_ExtensionLinePropertiesSubgraph.LineWidth, Subgraph_ExtensionLinePropertiesSubgraph.LineStyle, false, false, "");
            }
        ...
        if (sc.FormattedEvaluate(Low, sc.BaseGraphValueFormat, EQUAL_OPERATOR, sc.Low[Index], sc.BaseGraphValueFormat)) {
            ...
            if (InDrawExtensionLines) {
                ...
                sc.AddLineUntilFutureIntersection(Index, 2, Low, Subgraph_ExtensionLinePropertiesSubgraph.PrimaryColor, Subgraph_ExtensionLinePropertiesSubgraph.LineWidth, Subgraph_ExtensionLinePropertiesSubgraph.LineStyle, false, false, "");
            }
...
SCSFExport scsf_StudySubgraphsAdd(SCStudyInterfaceRef sc) {
    ...
    for (int Index = CalculationStartIndex; Index < sc.ArraySize; Index++) {
        ...
        if (Input_PerformAddWithZeroValue.GetYesNo() || (Value1 != 0.0 && Value2 != 0.0)) {
...
SCSFExport scsf_Inertia(SCStudyInterfaceRef sc) {
    ...
    Value1[sc.Index] = ((sc.Close[sc.Index] - sc.Open[sc.Index]) + 2 * (sc.Close[sc.Index - 1] - sc.Open[sc.Index - 1]) + 2 * (sc.Close[sc.Index - 2] - sc.Open[sc.Index - 2]) + (sc.Close[sc.Index - 3] - sc.Open[sc.Index - 3])) / 6;

    Value2[sc.Index] = ((sc.High[sc.Index] - sc.Low[sc.Index]) + 2 * (sc.High[sc.Index - 1] - sc.Low[sc.Index - 1]) + 2 * (sc.High[sc.Index - 2] - sc.Low[sc.Index - 2]) + (sc.High[sc.Index - 3] - sc.Low[sc.Index - 3])) / 6;
...
SCSFExport scsf_AskBidVolumeDifferenceBars(SCStudyInterfaceRef sc) {
    ...
    //  Pullback column
    if (Input_IncludePullbackColumn.GetYesNo() != 0 && (int)sc.PullbackVolumeAtPrice->GetNumberOfBars() > 0) {
        if (sc.IsFullRecalculation) {
...
SCSFExport scsf_VolumeAtPriceThresholdAlertV2(SCStudyInterfaceRef sc) {
    ...
    if (sc.Subgraph[1].PrimaryColor != sc.Subgraph[0].PrimaryColor || sc.Subgraph[1].DrawStyle != sc.Subgraph[0].DrawStyle || sc.Subgraph[1].LineWidth != sc.Subgraph[0].LineWidth) {
    ...
    for (int BarIndex = sc.UpdateStartIndex; BarIndex < sc.ArraySize; BarIndex++) {
        ...
        for (int PriceIndex = 0; PriceIndex < NumberOfPricesAtBarIndex; ++PriceIndex) {
            ...
            {
                if ((VolumeThresholdValue > 0 && p_VolumeAtPrice->BidVolume >= VolumeThresholdValue) || (VolumeThresholdValue == 0 && p_VolumeAtPrice->BidVolume == 0))
                    ConditionMet = true;

            } else if (ComparisonMethodIndex == 1)  //Ask Volume
            {
                if ((VolumeThresholdValue > 0 && p_VolumeAtPrice->AskVolume >= VolumeThresholdValue) || (VolumeThresholdValue == 0 && p_VolumeAtPrice->AskVolume == 0))
                    ConditionMet = true;
            } else if (ComparisonMethodIndex == 2)  //Total Volume
            {
                if ((VolumeThresholdValue > 0 && p_VolumeAtPrice->Volume >= VolumeThresholdValue) || (VolumeThresholdValue == 0 && p_VolumeAtPrice->Volume == 0))
                    ConditionMet = true;
            } else if (ComparisonMethodIndex == 3)  //Number of Trades
            {
                if ((VolumeThresholdValue > 0 && p_VolumeAtPrice->NumberOfTrades >= VolumeThresholdValue) || (VolumeThresholdValue == 0 && p_VolumeAtPrice->NumberOfTrades == 0))
                    ConditionMet = true;
            ...
            {
                if (
                    ((VolumeThresholdValue > 0 && p_VolumeAtPrice->BidVolume >= VolumeThresholdValue) || (VolumeThresholdValue == 0 && p_VolumeAtPrice->BidVolume == 0)) &&
                    ((AdditionalVolumeThresholdValue > 0 && p_VolumeAtPrice->AskVolume >= AdditionalVolumeThresholdValue) || (AdditionalVolumeThresholdValue == 0 && p_VolumeAtPrice->AskVolume == 0)))
                    ConditionMet = true;
            }
            ...
            if (ConditionMet) {
                ...
                if (Input_DrawExtensionLines.GetIndex() == 1  // All Alerts
                    ...
                    LineID++;
                           ...
                           && AdjacentAlertsGotGroup && !GotExtensionLineForGroup && sc.GetBarHasClosedStatus(BarIndex) == BHCS_BAR_HAS_CLOSED) {
                    ...
                    LineID++;
                           ...
                           && AdjacentAlertsGotGroup && sc.GetBarHasClosedStatus(BarIndex) == BHCS_BAR_HAS_CLOSED) {
...
SCSFExport scsf_LoadDaySessionOnlyAtOpen(SCStudyInterfaceRef sc) {
    ...
    if (r_PreviousCurrentTime != 0.0 && sc.UseSecondStartEndTimes && CurrentDateTime.GetTimeInSeconds() >= sc.StartTime1 && r_PreviousCurrentTime.GetTimeInSeconds() < sc.StartTime1) {
        sc.UseSecondStartEndTimes = 0;
    }

    // This code does not consider if the Start Time is 00:00:00. It is assumed to be greater than that.
    if (!r_PreviousCurrentTime.IsUnset() && !sc.UseSecondStartEndTimes && CurrentDateTime.GetTimeInSeconds() > sc.EndTime1 && r_PreviousCurrentTime.GetTimeInSeconds() <= sc.EndTime1) {
...
SCSFExport scsf_DivergenceDetector(SCStudyInterfaceRef sc) {
    ...
    if ((fabs(AngleDifference) >= ThresholdDegrees) || (MainGraphAngle > 0 && StudyAngle < 0 && fabs(AngleDifference) >= OppositeSlopeDivergenceThresholdDegrees) || (MainGraphAngle < 0 && StudyAngle > 0 && fabs(AngleDifference) >= OppositeSlopeDivergenceThresholdDegrees)) {
