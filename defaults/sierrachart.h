...
struct s_sc
{
	...
	{
		s_GetOHLCOfTimePeriod Data(StartDateTime, EndDateTime, Open, High, Low, Close, NextOpen, NumberOfBars, r_TotalTimeSpan);
	...
	{
		return Internal_GetOHLCForDate(Date.GetAsDouble(), Open, High, Low, Close);
	}
	...
	{
		return Internal_GetOpenHighLowCloseVolumeForDate(Date.GetAsDouble(), r_Open, r_High, r_Low, r_Close, r_Volume);
	}
	...
	{
		return Internal_GetOpenHighLowCloseVolumeForDate2(Date.GetAsDouble(), r_Open, r_High, r_Low, r_Close, r_Volume, IncludeFridayEveningSessionWithSundayEveningSession);
	}
	...
	{
		int Index = GetContainingIndexForSCDateTime(ChartNumber, DateTime.GetAsDouble());
	...
	{
		return InternalGetStartOfPeriodForDateTime(DateTime.GetAsDouble(), TimePeriodType, TimePeriodLength, PeriodOffset, NewPeriodAtBothSessionStarts);
	}
	...
	{
		return InternalGetStartOfPeriodForDateTime(DateTime.GetAsDouble(), TimePeriodType, TimePeriodLength, PeriodOffset, 0);
	}
	...
	{
		return InternalConvertDateTimeFromChartTimeZone(DateTime.GetAsDouble(), TimeZonePOSIXString);
	}

	SCDateTime ConvertDateTimeFromChartTimeZone(const SCDateTime& DateTime, TimeZonesEnum TimeZone)
	{
		return InternalConvertDateTimeFromChartTimeZone(DateTime.GetAsDouble(), GetPosixStringForTimeZone(TimeZone));
	}
	...
	*/
	...
	int GetBarsSinceLastTradeOrderEntry()
	{
		...
		int BarsSinceEntry = ArraySize - GetContainingIndexForSCDateTime(ChartNumber, PositionData.LastEntryDateTime.GetAsDouble()) - 1;
	...
	int GetBarsSinceLastTradeOrderExit()
	{
		...
		int BarsSinceExit = ArraySize - GetContainingIndexForSCDateTime(ChartNumber, PositionData.LastExitDateTime.GetAsDouble()) - 1;
	...
	bool IsIsSufficientTimePeriodInDate(const SCDateTime& DateTime, float Percentage)
	{
		...
		int  FirstIndex = GetContainingIndexForSCDateTime(ChartNumber, StartDateTime.GetAsDouble());
		int  LastIndex = GetContainingIndexForSCDateTime(ChartNumber, EndDateTime.GetAsDouble());
	...
	double (SCDLLCALL* GetStartDateTimeForTradingDate)(const SCDateTime& TradingDate);
	...
	//Parameters reviewed for safety with different compilers
	double (SCDLLCALL* InternalGetStartOfPeriodForDateTime)(const SCDateTime& DateTime, unsigned int TimePeriodType, int TimePeriodLength, int PeriodOffset, int NewPeriodAtBothSessionStarts);
	...
	double (SCDLLCALL* InternalConvertDateTimeFromChartTimeZone)(const SCDateTime& DateTime, const char* TimeZonePosixString);
