...
SCSFExport scsf_TradingExample(SCStudyInterfaceRef sc) {
    ...
    if (sc.SetDefaults) {
        ...
        sc.StudyDescription = "This study function is an example of how to use the ACSIL Trading Functions.
		 This function will display a simple moving average and perform a Buy Entry when the Last price crosses
		  the moving average from below and a Sell Entry when the Last price crosses the moving average from above. 
		  A new entry cannot occur until the Target or Stop has been hit. When an order is sent, a corresponding 
		  arrow will appear on the chart to show that an order was sent. This study will do nothing until the
    ...
    else if (PositionData.PositionQuantity > 0 && (LastTradePrice <= PositionData.AveragePrice - Input_StopValue.GetFloat() ||
...
SCSFExport scsf_TradingExampleWithAttachedOrdersDirectlyDefined(SCStudyInterfaceRef sc) {
    ...
    if (sc.SetDefaults) {
        ...
        sc.StudyDescription = "This study function is an example of how to use the ACSIL Trading Functions. 
                                  This example uses Attached Orders defined directly within this function.This function will display a simple moving average and
                                  perform a Buy Entry
                                      when the Last price crosses the moving average from below and
                                  a Sell Entry when the Last price
                                      crosses the moving average from above.A new entry cannot occur until the Target or
                              Stop has been hit.When an order is sent,
        a corresponding arrow will appear on the chart to show that an order was sent.This study will do nothing until the Enabled Input is set to Yes.";
...
SCSFExport scsf_TradingSystemStudySubgraphCrossover(SCStudyInterfaceRef sc) {
    ...
    if (sc.SetDefaults) {
        ...
        sc.StudyDescription =
            "A trading system that enters a new position or \
    ...
    if (Input_EvaluateOnBarCloseOnly.GetYesNo() && sc.GetBarHasClosedStatus() == BHCS_BAR_HAS_NOT_CLOSED) {
        return;
    }
    ...
    if (sc.CrossOver(StudyLine1, StudyLine2) == CROSS_FROM_BOTTOM) {
        ...
        if (!sc.IsFullRecalculation && !sc.DownloadingHistoricalData) {
            if (Input_ActionOnCrossoverWhenInPosition.GetIndex() == 1 && PositionData.PositionQuantity < 0) {
                s_SCNewOrder NewOrder;
            ...
            } else if (Input_ActionOnCrossoverWhenInPosition.GetIndex() == 2 && PositionData.PositionQuantity < 0) {
                sc.SupportReversals = true;
                s_SCNewOrder NewOrder;
...
SCSFExport scsf_TradingExampleGetPositionData(SCStudyInterfaceRef sc) {
    ...
    PositionDataMessage.Format("MaximumOpenPositionProfit: %f. MaximumOpenPositionLoss: %f.", PositionData.MaximumOpenPositionProfit, PositionData.MaximumOpenPositionLoss);
...
SCSFExport scsf_TradingExampleOrdersForDifferentSymbolAndAccount(SCStudyInterfaceRef sc) {
    ...
    if (sc.ChartIsDownloadingHistoricalData(Input_ReferenceChartForPrice.GetChartNumber()) || sc.IsFullRecalculation || sc.ServerConnectionState != SCS_CONNECTED)
        return;
    ...
    sc.GetChartArray(Input_ReferenceChartForPrice.GetChartNumber(), SC_LAST, LastPriceArrayFromChartToTrade);
    ...
    if (PositionData.PositionQuantity == 0 && PositionData.WorkingOrdersExist == 0 && r_InternalOrderID == 0) {
        ...
        NewOrder.OrderType = SCT_ORDERTYPE_LIMIT;
        NewOrder.TimeInForce = SCT_TIF_DAY;
        NewOrder.Price1 = LastPriceArrayFromChartToTrade[LastIndexOfArrayFromChartToTrade] - 10 * sc.TickSize;
...
SCSFExport scsf_TradingExampleWithSingleAttachedOrder(SCStudyInterfaceRef sc) {
    ...
    if (sc.SetDefaults) {
        ...
        sc.StudyDescription =
            "This study function is an example of how to use the ACSIL Trading Functions.\
...
SCSFExport scsf_TradingScaleInExample(SCStudyInterfaceRef sc) {
    ...
    // Buy when the last price crosses the moving average from below.
    if (ConditionForInitialBuyEntry && PositionData.PositionQuantity == 0 && !PositionData.WorkingOrdersExist) {
        // Create an s_SCNewOrder object.
        s_SCNewOrder NewOrder;
        NewOrder.OrderQuantity = 1;
        NewOrder.OrderType = SCT_ORDERTYPE_MARKET;
    ...
    } else if (ConditionForSecondBuyEntry && PositionData.PositionQuantity == 1 && !PositionData.NonAttachedWorkingOrdersExist) {
        // Create an s_SCNewOrder object.
        s_SCNewOrder NewOrder;
        NewOrder.OrderQuantity = 1;
        NewOrder.OrderType = SCT_ORDERTYPE_MARKET;
...
SCSFExport scsf_ACSILTradingTest2(SCStudyInterfaceRef sc) {
    ...
    // Run the below code only on the last bar
    if (sc.Index < sc.ArraySize - 1 || sc.IsFullRecalculation)
        return;
