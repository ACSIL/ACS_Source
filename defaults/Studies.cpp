...
SCSFExport scsf_OutsideBar(SCStudyInterfaceRef sc) {
    ...
    if (Input_IncludeEqualHighLow.GetYesNo() == 0 &&
        High[sc.Index] > High[sc.Index - 1] && Low[sc.Index] < Low[sc.Index - 1]) {
        Subgraph_OutsideBar[sc.Index] = High[sc.Index];
    } else if (Input_IncludeEqualHighLow.GetYesNo() != 0 &&
               ((High[sc.Index] >= High[sc.Index - 1] && Low[sc.Index] < Low[sc.Index - 1]) || (High[sc.Index] > High[sc.Index - 1] && Low[sc.Index] <= Low[sc.Index - 1]))) {
        Subgraph_OutsideBar[sc.Index] = High[sc.Index];
    } else {
...
void TimeAndSales(SCStudyInterfaceRef sc, TimeAndSalesStudyFlagsEnum Flag) {
    ...
    for (int TSIndex = TimeSalesSize - 1; TSIndex >= 0; --TSIndex) {
        ...
        if (Flag == TIME_AND_SALES_STUDY_PRICE || Flag == TIME_AND_SALES_STUDY_VOLUME || Flag == TIME_AND_SALES_STUDY_TIME) {
            if (Volume < (unsigned int)InVolumeMinFilter.GetInt() || (Volume > (unsigned int)InVolumeMaxFilter.GetInt() && InVolumeMaxFilter.GetInt() > 0) || TimeAndSalesRecord.Type == SC_TS_BIDASKVALUES)
                continue;
        } else if (Flag == TIME_AND_SALES_STUDY_BID_AND_ASK_PRICES || Flag == TIME_AND_SALES_STUDY_BID_SIZE || Flag == TIME_AND_SALES_STUDY_ASK_SIZE || Flag == TIME_AND_SALES_STUDY_TOTAL_BID_SIZE_DEPTH || Flag == TIME_AND_SALES_STUDY_TOTAL_ASK_SIZE_DEPTH) {
        ...
        } else if (Flag == TIME_AND_SALES_STUDY_BID_SIZE)  // Bid size
        {
            ...
            if (BidSize < InVolumeMinFilter.GetInt() || (InVolumeMaxFilter.GetInt() > 0 && BidSize > InVolumeMaxFilter.GetInt()))
                continue;
        ...
        } else if (Flag == TIME_AND_SALES_STUDY_ASK_SIZE)  // Ask size
        {
            ...
            if (AskSize < InVolumeMinFilter.GetInt() || (InVolumeMaxFilter.GetInt() > 0 && AskSize > InVolumeMaxFilter.GetInt()))
                continue;
            ...
            if (TotalBidSizeDepth < InVolumeMinFilter.GetInt() || (InVolumeMaxFilter.GetInt() > 0 && TotalBidSizeDepth > InVolumeMaxFilter.GetInt()))
                continue;
            ...
            if (TotalAskSizeDepth < InVolumeMinFilter.GetInt() || (InVolumeMaxFilter.GetInt() > 0 && TotalAskSizeDepth > InVolumeMaxFilter.GetInt()))
                continue;
...
SCSFExport scsf_HeikinAshi(SCStudyInterfaceRef sc) {
    ...
    if (sc.Index == 0)
        ...
        Subgraph_Open[sc.Index] = (Subgraph_Open[sc.Index - 1] + Subgraph_Last[sc.Index - 1]) / 2.0f;
    }
...
SCSFExport scsf_DepthBarsMaxDepthQuantity(SCStudyInterfaceRef sc) {
    ...
    do {
        ...
        const int MaxAskQuantityAtPriceTick = p_DepthBars->GetMaxAskQuantity(sc.Index, PriceTickIndex);
