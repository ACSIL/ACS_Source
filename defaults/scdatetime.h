...
----------------------------------------------------------------------------*/
DEPRECATED("Usage of TIME_PART_MS is deprecated. Use portable SCDateTimeMS::GetTime() instead.")
...
The SCDateTimeMS object wraps a single 8-byte integer value representing
a complete date and time in form of a number of microseconds elapsed from
the SC epoch, 30 December 1899 00:00:00.
	...
	: public SCDateTime
{
	public:
		...
		#ifdef SCDATETIME_SUPPORT_CHRONO
		template<typename t_Rep, typename t_Period>
			SCDateTimeMS(const std::chrono::duration<t_Rep, t_Period> Duration);
		SCDateTimeMS(const std::chrono::system_clock::time_point TimePoint);
		#endif
...
{
	return abs(GetInternalDateTime() - DateTime.GetInternalDateTime()) < MICROSECONDS_PER_SECOND / 2;
}
...
inline bool SCDateTime::operator < (const SCDateTime& DateTime) const
{
	return GetInternalDateTime() < DateTime.GetInternalDateTime() - MICROSECONDS_PER_SECOND / 2;
}
...
{
	return GetInternalDateTime() == DateTimeMS.GetInternalDateTime();
}
...
{
	return TIME_PART(GetInternalDateTime());
}
...
{
	SCDateTimeMS DateTimeMS = GetAbsoluteValue();
	DateTimeMS.m_dt %= MICROSECONDS_PER_DAY;	// remove date part
	return DateTimeMS;
}
...
{
	const int64_t ThisDT = ADJUST_ROUND_SECOND(GetInternalDateTime());
	const int64_t ThatDT = ADJUST_ROUND_SECOND(DateTime.GetInternalDateTime());

	const int64_t ThisHour = ThisDT / MICROSECONDS_PER_HOUR;
	const int64_t ThatHour = ThatDT / MICROSECONDS_PER_HOUR;
...
inline bool SCDateTime::IsSameTimeToMinute(const SCDateTime& DateTime) const
{
	...
	const int64_t ThisMinute = ThisDT / MICROSECONDS_PER_MINUTE;
	const int64_t ThatMinute = ThatDT / MICROSECONDS_PER_MINUTE;
...
inline bool SCDateTime::IsSameTimeToSecond(const SCDateTime& DateTime) const
{
	...
	const int64_t ThisSecond = ThisDT / MICROSECONDS_PER_SECOND;
	const int64_t ThatSecond = ThatDT / MICROSECONDS_PER_SECOND;
...
inline SCDateTimeMS::SCDateTimeMS(const SCDateTimeMS& DateTime)
	: SCDateTime()
{
	GetInternalDateTime() = DateTime.GetInternalDateTime();
}

/*==========================================================================*/
inline SCDateTimeMS::SCDateTimeMS(int Date, int Time)
	: SCDateTime(Date, Time)
{
}
...
{
	this->SCDateTime::operator=(DateTime);
	return *this;
}
...
{
	return *this == SCDateTimeMS(Date, 0);
}
...
{
	return GetInternalDateTime() == SCDateTimeMS(DateTime).GetInternalDateTime();
}
...
/*==========================================================================*/
inline void SCDateTimeMS::GetDateTimeYMDHMS(int& Year, int& Month, int& Day, int& Hour, int& Minute, int& Second) const
{
	int Millisecond = 0;
	DATETIME_TO_YMDHMS_MS(GetInternalDateTime(), Year, Month, Day, Hour, Minute, Second, Millisecond);
}

/*==========================================================================*/
inline int SCDateTimeMS::GetDate() const
{
	return DATE_PART_MS(GetInternalDateTime());
}
...
/*==========================================================================*/
inline int SCDateTimeMS::GetTimeInSeconds() const
{
	return SCDateTimeMS::GetTime();
}

/*==========================================================================*/
inline int SCDateTimeMS::GetDayOfWeek() const
{
	return DAY_OF_WEEK(GetDate());
}
...
/*==========================================================================*/
inline bool operator == (int IntDateTime, const SCDateTimeMS& DateTime)
{
	return SCDateTimeMS(IntDateTime, 0) == DateTime;
}
...
/*==========================================================================*/
inline bool operator == (double dt, const SCDateTimeMS& DateTime)
{
	return SCDateTimeMS(dt) == DateTime;
}
...
/*==========================================================================*/
inline SCDateTimeMS GetNextMillisecondForSameSecondDateTime_AllowSkippedMilliseconds(const SCDateTimeMS PreviousDateTime, SCDateTimeMS NextDateTime)
{
	//Purpose: We need to make a new record timestamp unique by using milliseconds.
		...
		&& NextDateTime <= PreviousDateTime)// Skip this if NextDateTime is greater than PreviousDateTime
	{
		...
		if (Milliseconds >= MAXIMUM_INTRADAY_DATA_FILE_MILLISECONDS)
			...
			NextDateTime = PreviousDateTime + SCDateTimeMS::MILLISECONDS(1);
	}
...
inline SCDateTimeMS GetNextMicrosecondForSameMillisecondDateTime(const SCDateTimeMS& PreviousDateTime, SCDateTimeMS NextDateTime)
{
