...
SCDLLName("Logger")
    ...
    int dom{};
    if (dom_quotes_ab > abs(dom_quotes_ba))
        dom = dom_quotes_ab;
    else
        dom = dom_quotes_ba;
    ...
    log.Format("Entry DateTime: %i-%i-%i, %0.2i:%0.2i:%0.2i,  
               Entry price
               : % 3.2f, Symbol
               : % s, Quantity
               : % 3.0f, ATR
               ...
               : % i ", 
                     Year,
                 Month, Day, Hour, Minute, Second, PositionData.AveragePrice,
                 symbol, qty, atr_value, wtr_value, dom);
